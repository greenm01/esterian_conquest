...
DEFAULT_ROOT = Path("/tmp/nc-dash-lab")
...
def run_nc_dash(game_dir: Path, release: bool) -> None:
    cmd = ["cargo", "run", "-q"]
    if release:
        cmd.append("--release")
    cmd.extend(["-p", "nc-dash", "--", str(game_dir)])
    subprocess.run(cmd, cwd=RUST_DIR, check=True, env=cargo_env())


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Seed a four-map nc-dash stress lab and print launch commands."
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        ...
        help=f"Base seed forwarded to nc-cli harness seed-nc-dash-lab. Default: {DEFAULT_SEED_BASE}.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Launch one seeded nc-dash campaign immediately after setup.",
    )
    parser.add_argument(
        "--release",
        action="store_true",
        help="When used with --launch, run nc-dash with cargo --release.",
    )
...
def print_lab_summary(root: Path) -> None:
    print()
    print(f"Seeded nc-dash lab at {root}")
    ...
    for slug, players, map_size in LAB_PROFILES:
        ...
        print(f"    cargo run -q -p nc-dash -- {game_dir}")
...
def main() -> None:
    ...
    if args.launch:
        print()
        print(f"Launching nc-dash for {args.launch}...")
        run_nc_dash(root / args.launch, args.release)
