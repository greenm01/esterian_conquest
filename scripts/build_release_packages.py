...
PACKAGE_SPECS = (
    ...
    PackageSpec(
        ...
        notes_summary=(
            "This package swaps in decrypted plain-MZ executables while "
            "keeping the same filenames and game layout."
        ),
        emulator_notes=(
            "Use this bundle when you want the stub-free binaries for "
            "inspection or emulator comparison. It keeps the same game "
            "directory and dropfile shape as the classic bundle."
        ),
    ),
)
...
def releases_readme() -> str:
    return (
        ...
        "Current bundles:\n\n"
        "- `ec-v1.5-classic-demo.zip`: original packed DOS executables plus a "
        "known-good local-console `CHAIN.TXT`\n"
        "- `ec-v1.5-unlocked-demo.zip`: unlocked plain-MZ executables in the "
        "same layout and with the same `CHAIN.TXT`\n\n"
