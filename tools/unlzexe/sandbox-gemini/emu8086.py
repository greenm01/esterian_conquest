...
def main():
    ...
    try:
        ...
        while emu.insn_count < args.max_insns:
            cs = emu.sregs['cs']
            trace.append(f"{cs:04x}:{emu.ip:04x} AX={emu.regs['ax']:04x} BX={emu.regs['bx']:04x} CX={emu.regs['cx']:04x} DX={emu.regs['dx']:04x} SI={emu.regs['si']:04x} DI={emu.regs['di']:04x} BP={emu.regs['bp']:04x} SP={emu.regs['sp']:04x} FL={emu.flags:04x}")
            if len(trace) > 100: trace.pop(0)
