...
TARGET = Path("/tmp/ecgame-first-open-regs")
...
def main() -> None:
    ...
    try:
        ...
        (TARGET / "regs.txt").write_text(text, encoding="utf-8", errors="replace")

        ds_match = re.search(r"\bDS=([0-9A-F]{4})\b", text)
        dx_match = re.search(r"\bEDX=0000([0-9A-F]{4})\b", text)
        if not dx_match:
            dx_match = re.search(r"\bDX=([0-9A-F]{4})\b", text)
        if not ds_match or not dx_match:
            raise RuntimeError(f"Unable to parse DS:DX from register dump:\n{text}")

        segment = ds_match.group(1)
        offset = dx_match.group(1)
