...
def setup_case(case_dir: Path, tax_rate: int, colony: dict[str, object], with_base: bool) -> None:
    ...
    run_ec_cli(["db-import", str(case_dir)])
    run_ec_cli(["player-join", str(case_dir), "1", "SYSOP", "Alpha", "Foundation"])
    run_ec_cli(["player-tax", str(case_dir), "1", str(tax_rate)])
    ...
    run_ec_cli(["planet-owner", str(case_dir), str(record), "1"])
    run_ec_cli(["planet-name", str(case_dir), str(record), name])
    run_ec_cli(["planet-present", str(case_dir), str(record), str(present)])
    run_ec_cli(["planet-stored", str(case_dir), str(record), "0"])
    run_ec_cli(["planet-stats", str(case_dir), str(record), str(armies), str(batteries)])

    if with_base:
        run_ec_cli(["guard-starbase-onebase", str(case_dir), str(x), str(y)])
    ...
    for colony in COLONIES:
        ...
        for tax_rate in args.tax:
            ...
            for mode, with_base in [("plain", False), ("base", True)]:
                ...
                pre_report = run_ec_cli(["economy-report", str(case_dir), "1"])
                run_ec_cli(["db-export", str(case_dir), str(case_dir)])
                oracle_log = run_oracle(case_dir)
                run_ec_cli(["db-import", str(case_dir)])
                post_report = run_ec_cli(["economy-report", str(case_dir), "1"])
