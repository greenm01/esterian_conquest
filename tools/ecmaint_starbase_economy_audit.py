...
def setup_runtime_probe(runtime_dir: Path, tax_rate: int, seed: int) -> str:
    ...
    log.append(
        run_ec_cli(["player-name", str(runtime_dir), "2", "HECATE", "RedHorizon"]).strip()
    )
    log.append(run_ec_cli(["player-name", str(runtime_dir), "3", "ORION", "Vela"]).strip())
    log.append(run_ec_cli(["player-name", str(runtime_dir), "4", "TESS", "Helios"]).strip())
    for player in range(1, 5):
        log.append(
            run_ec_cli(["player-tax", str(runtime_dir), str(player), "50"]).strip()
        )
    log.append(
        run_ec_cli(
    ...
    log.append(
        run_ec_cli(["fleet-order", str(runtime_dir), "2", "3", "1", "12", "12"]).strip()
    )
    log.append(
        run_ec_cli(
...
def main() -> int:
    ...
    for tax_rate in args.tax:
        ...
        pre_report = run_ec_cli(["economy-report", str(runtime_dir), "1"])
        (case_root / "economy-before.txt").write_text(pre_report, encoding="utf-8")

        export_log = run_ec_cli(["db-export", str(runtime_dir), str(classic_dir)])
        ...
        import_log = run_ec_cli(["db-import", str(classic_dir)])
        post_report = run_ec_cli(["economy-report", str(classic_dir), "1"])
