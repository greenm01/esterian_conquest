...
use nc_daemon::config::daemon_config::DaemonConfig;
use nc_daemon::status::{collect, render};
use nc_data::hosted::{
...
fn collect_status_aggregates_games_and_queues() {
    ...
    let config = DaemonConfig {
        ...
        identity_path: temp.path().join("daemon.nsec"),
...
fn render_human_mentions_totals_and_games() {
    ...
    assert!(rendered.contains("nc-daemon status"));
