...
impl DaemonIdentity {
    ...
    pub fn parse(content: &str) -> Result<Self, IdentityError> {
        ...
        if let Some(nsec_line) = content.lines().find(|l| l.starts_with("nsec1")) {
            ...
            return Ok(DaemonIdentity { npub, nsec });
        }
...
pub fn generate_identity() -> Result<DaemonIdentity, IdentityError> {
    ...
    Ok(DaemonIdentity { npub, nsec })
}

pub fn save_identity(path: &PathBuf, identity: &DaemonIdentity) -> Result<(), IdentityError> {
