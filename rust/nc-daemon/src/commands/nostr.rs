...
fn run_init(args: &[&str]) -> Result<(), Box<dyn std::error::Error>> {
    let mut identity_path = PathBuf::from("/etc/nc-daemon/daemon.nsec");
    ...
    identity::save_identity(&identity_path, &identity)?;

    println!("Generated daemon identity:");
...
fn print_usage() {
    println!("Usage:");
    println!("  nc-daemon nostr init [--path <path>]");
    println!();
    println!("Options:");
    println!("  --path <path>  Path to save identity (default: /etc/nc-daemon/daemon.nsec)");
}
