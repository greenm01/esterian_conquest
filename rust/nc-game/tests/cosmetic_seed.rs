...
use nc_compat::import_directory_snapshot_with_seed;
use nc_data::{CampaignStore, DEFAULT_CAMPAIGN_DB_NAME};
use nc_game::app::{App, AppConfig};
use nc_game::screen::{MainMenuScreen, PLAYFIELD_HEIGHT, ScreenId};
...
fn seeded_fixture_copy(seed: u64) -> PathBuf {
    let root = temp_dir("nc-game-cosmetic-seed");
...
fn startup_splash_styles_are_deterministic_for_campaign_seed() {
    ...
    let buffer_a = nc_game::domains::startup::views::render(&mut app_a).expect("render app a");
    let buffer_b = nc_game::domains::startup::views::render(&mut app_b).expect("render app b");
