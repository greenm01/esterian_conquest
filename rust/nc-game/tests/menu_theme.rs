...
#[test]
fn featured_menu_hotkey_cells_are_used_for_planet_and_fleet_lists() {
    let _guard = menu_theme_test_guard();
    theme::apply_default_theme();

    let featured_style = classic::menu_featured_hotkey_style();
    ...
    assert_eq!(planet_buffer.row(3)[planet_col].style, featured_style);
    ...
    assert_eq!(fleet_buffer.row(4)[fleet_col].style, featured_style);
