...
use nc_data::{CampaignStore, HostedSeat};
...
pub fn load_hosted_game(dir: &Path) -> Result<HostedGameEntry, String> {
    ...
    let store = CampaignStore::open_default_in_dir(dir)
        .map_err(|err| format!("cannot open {}: {err}", dir.join("ncgame.db").display()))?;
    let seats = store
        .hosted_seats()
    ...
    if seats.is_empty() {
        ...
        if legacy_roster.exists() {
            return Err(format!(
                "{} still uses roster.kdl; run `nc-sysop nostr migrate-roster --dir {}` first",
                dir.display(),
                dir.display()
            ));
        }
        return Err(format!(
            "{} has no hosted seats in ncgame.db; initialize hosted seats before serving",
            dir.display()
        ));
    }
    let settings = store
