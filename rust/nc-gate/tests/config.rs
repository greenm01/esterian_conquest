...
use nc_gate::config::io::{config_path, load_config, parse_config_str, render_config, save_config};
use nc_gate::config::{AuthKeysMethod, DEFAULT_NC_GAME_PATH};
...
fn load_config_from_file() {
    let dir = std::env::temp_dir().join("nc-gate-config-test");
...
fn render_and_save_config_round_trip() {
    let dir = std::env::temp_dir().join("nc-gate-config-save-test");
    fs::create_dir_all(&dir).unwrap();
    let path = dir.join("config.kdl");
    let cfg = parse_config_str(CANONICAL_CONFIG).expect("parse failed");
...
fn load_config_missing_file_is_error() {
    let path = PathBuf::from("/tmp/nc-gate-no-such-config-file.kdl");
...
fn config_path_returns_xdg_config_home_when_set() {
    ...
    // still prefers /etc/nc-gate when that directory exists.
    let tmp = std::env::temp_dir().join("nc-gate-xdg-test");
    ...
    if PathBuf::from("/etc/nc-gate").exists() {
        assert_eq!(path, PathBuf::from("/etc/nc-gate/config.kdl"));
    } else {
        assert_eq!(path, tmp.join("nc-gate").join("config.kdl"));
    }
}
...
fn config_path_falls_back_to_home_config() {
    ...
    // Path should be under HOME/.config/nc-gate/config.kdl (unless /etc/nc-gate exists).
    if !PathBuf::from("/etc/nc-gate").exists() {
        assert_eq!(
            path,
            PathBuf::from(&home)
