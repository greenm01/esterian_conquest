...
use nc_ui::modal::{ModalTheme, Rect, modal_content_rect};
use nc_ui::prompt::{draw_command_line_prompt_text_at, draw_command_line_prompt_text_at_col};
...
fn render_help_overlay(buffer: &mut PlayfieldBuffer, topic: HelpTopic) {
    let spec = topic.spec();
    let mut lines =
        nc_ui::modal::format_help_rows(spec.rows.iter().map(|row| (row.command, row.description)));
...
fn render_modal_box(
    ...
    title: &str,
    lines: &[String],
    body_style: nc_ui::buffer::CellStyle,
) -> Rect {
    nc_ui::modal::render_modal_box(
        buffer,
        title,
        lines,
        ModalTheme {
