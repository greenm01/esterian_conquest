...
use nc_connect::cache::{CachedGame, CachedGameStatus, GameCache};
use nc_connect::config::ConnectConfig;
...
use nc_connect::picker::refresh::PendingRefreshRequest;
use nc_connect::picker::relay::RelayPromptAction;
use nc_connect::picker::render::{Rect, centered_rect, matrix_glyph, short_npub, truncate};
...
fn unique_temp_path(name: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    std::env::temp_dir().join(format!("nc-connect-{name}-{nanos}.kdl"))
}
...
fn unique_temp_dir(name: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    std::env::temp_dir().join(format!("nc-connect-{name}-{nanos}"))
}
...
fn help_overlay_renders_left_aligned_title_and_commands() {
    ...
    let buffer = nc_connect::picker::render::render_buffer(&state, None, 82, 27);
...
fn empty_picker_keeps_one_body_row_and_command_line_under_table() {
    let state = make_state(vec![]);
    let buffer = nc_connect::picker::render::render_inner_buffer(&state, None);
...
fn join_code_popup_shows_code_input() {
    use nc_connect::picker::overlay::PickerOverlay;
    let mut state = make_state(vec![make_game("a", Some("2026-03-26T00:00:00Z"))]);
...
fn keychain_detail_popup_renders_full_backup_material_and_copy_hints() {
    ...
    let buffer = nc_connect::picker::render::render_buffer(&state, Some(&session), 82, 27);
...
fn persist_maps_root_updates_state_and_writes_config() {
    ...
    let config = nc_connect::config::load_config_from(&config_path).expect("load config");
...
fn larger_terminal_keeps_space_hint_in_command_line_not_outside_border() {
    ...
    let buffer = nc_connect::picker::render::render_buffer(&state, None, 100, 30);
...
fn gui_inner_picker_buffer_uses_plain_80x25_canvas_without_outer_shell() {
    let state = make_state(vec![make_game("a", Some("2026-03-26T00:00:00Z"))]);
    let session = make_session(Some("Desk Alias"));
    let buffer = nc_connect::picker::render::render_inner_buffer(&state, Some(&session));
