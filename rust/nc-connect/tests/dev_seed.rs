...
use nc_connect::cache;
use nc_connect::config::load_config_from;
use nc_connect::dev_seed::{
    SeedLocalhostFixtureOptions, SeedUiOptions, seed_localhost_fixture_to_paths, seed_ui_to_paths,
};
use nc_connect::keychain::io::load_keychain_from;
use nc_connect::keychain::{IdentityType, active_identity_npub};
...
fn temp_dir(name: &str) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let path = std::env::temp_dir().join(format!("nc-connect-{name}-{unique}"));
