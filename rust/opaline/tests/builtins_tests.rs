...
#[test]
fn load_by_name_normalizes_legacy_underscore_ids() {
    let legacy = builtins::load_by_name("tokyo_night").expect("legacy id loads");
    let canonical = builtins::load_by_name("tokyo-night").expect("canonical id loads");
    assert_eq!(legacy.meta.name, canonical.meta.name);
}
...
fn theme_info_load_prefers_path_over_builtin_id() {
    ...
    let path = temp_dir.join("dracula.kdl");
    fs::write(
        &path,
        r##"
meta name="Shadow Dracula" variant="dark" author="local"
palette "shadow" "#010203"
"##,
    )
...
"##,
    ...
    fs::write(
        app_dir.join("opaline-local.kdl"),
        r##"
meta name="Opaline Local" variant="light" author="local"
...
"##,
    ...
    let _ = fs::remove_file(base_dir.join("dracula.kdl"));
    let _ = fs::remove_file(app_dir.join("opaline-local.kdl"));
...
fn later_discovery_dirs_override_earlier_ones() {
    ...
    fs::write(
        base_dir.join("shared.kdl"),
        r##"
meta name="Base Shared" variant="dark"
...
"##,
    ...
    fs::write(
        app_dir.join("shared.kdl"),
        r##"
meta name="App Shared" variant="light"
palette "accent" "#abcdef"
"##,
    )
    .expect("write app theme");
    ...
    let _ = fs::remove_file(base_dir.join("shared.kdl"));
    let _ = fs::remove_file(app_dir.join("shared.kdl"));
