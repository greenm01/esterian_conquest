...
/// Default `config.kdl` content bundled into `nc-data`.
///
/// Callers (e.g. `nc-game`) bootstrap this into a new game directory when
...
/// `nc-game` reads this file and applies any differing values into
/// `ncgame.db` so the engine and TUI always see the current config.
