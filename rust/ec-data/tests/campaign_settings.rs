...
fn campaign_settings_round_trip_and_update_runtime_policy_bytes() {
    let (root, store, _) = seeded_store("ec-data-settings-roundtrip");
...
fn session_leases_enforce_one_live_seat_per_player_and_expire() {
    let (root, store, _) = seeded_store("ec-data-session-leases");
    store
    ...
    assert!(matches!(err, ec_data::SessionLeaseError::SeatBusy { .. }));

    let active = store
        .activate_session_lease("token-1", 120, 60)
        .expect("activate lease");
    assert_eq!(active.state, ec_data::SessionLeaseState::Active);
...
fn campaign_settings_can_coexist_with_hosted_seats() {
    let (root, store, _) = seeded_store("ec-data-settings-hosted-seats");
    store
        .save_campaign_settings(&CampaignSettings::new("friday-night", "Friday Night EC"))
        .expect("save settings");
    store
