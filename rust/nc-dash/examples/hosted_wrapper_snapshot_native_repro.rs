use nc_dash::{
    NativeLaunchOptions, run_hosted_wrapper_snapshot_native_repro,
...
fn print_usage() {
    println!(
        "Usage: cargo run -p nc-dash --example hosted_wrapper_snapshot_native_repro -- --snapshot <path> [--backend auto|wayland|x11] [--windowed|--fullscreen] [--diagnostic]"
    );
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "hosted_wrapper_snapshot_native_repro: backend={}, window_mode={}, pid={}",
    ...
    run_hosted_wrapper_snapshot_native_repro(&snapshot, native)
}
