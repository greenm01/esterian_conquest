use nc_dash::{
    NativeLaunchOptions, run_unlocked_open_game_native_repro,
...
fn print_usage() {
    println!(
        "Usage: cargo run -p nc-dash --example unlocked_open_game_native_repro -- --snapshot <path> --password <password> [--relay <url>] [--backend auto|wayland|x11] [--windowed|--fullscreen] [--diagnostic] [--freeze-live-updates] [--no-live-session] [--live-no-private] [--no-hosted-sessions]"
    );
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "unlocked_open_game_native_repro: backend={}, window_mode={}, pid={}",
    ...
    run_unlocked_open_game_native_repro(&password, relay_override, &snapshot, native)
}
