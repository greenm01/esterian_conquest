...
use nc_data::{
    BaseRecord, CoreGameData, FleetRecord, GameStateBuilder, IntelTier, Order,
...
use nc_nostr::state_sync::{
    GameState, HostedFleetShips, HostedOwnedFleet, HostedWorldState,
};
...
pub fn build_hosted_dash_app(
    ...
    let queued_mail = snapshot
        ...
        .collect::<Vec<_>>();
    let planet_intel_snapshots = build_planet_intel_snapshots(snapshot);
    let player_activity_states = build_player_activity_states(snapshot, player_count);
    let player_lifecycle_states = build_player_lifecycle_states(player_count);
    ...
    let mut app = DashApp::new(
        ...
        snapshot.player_seat as usize,
    );
    app.player_war_stats = PlayerWarStatsState::for_player(snapshot.player_seat as usize);
...
fn build_player_lifecycle_states(player_count: u8) -> Vec<PlayerLifecycleState> {
