...
pub fn run(args: &[&str]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config_path = config_path
        .or_else(|| std::env::var("NC_HOST_CONFIG").ok().map(PathBuf::from))
        .unwrap_or_else(HostConfig::default_config_path);
...
fn print_usage() {
    println!("Usage: nc-host status [--config <path>] [--root <path>] [--json]");
}
