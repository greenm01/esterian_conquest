...
///
/// Resolution order:
/// 1. System-level: `/etc/ec-gate/identity.kdl` (if the directory exists and
///    the process can write to it — i.e., we're running as root / in deployment).
/// 2. User-level: `~/.local/share/ec-gate/identity.kdl` (default for dev /
...
pub fn identity_path() -> PathBuf {
    ...
    base.join("ec-gate").join("identity.kdl")
}
