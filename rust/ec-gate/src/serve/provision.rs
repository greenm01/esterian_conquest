...
//!   `ec-gate-keys` helper prints all files in that directory; sshd calls it
//!   via `AuthorizedKeysCommand`.
//!
//! * **`File`** — append/remove from `authorized_keys` directly.  Each entry
//!   is bracketed by `# ec-gate-begin <key_id>` / `# ec-gate-end <key_id>`
...
//
// The `ec-gate-keys` helper reads all `.key` files and prints only the
...
//   # ec-gate-begin <key_id> expires=<ts>
//   <authorized-keys-line>
//   # ec-gate-end <key_id>

const BEGIN_PREFIX: &str = "# ec-gate-begin ";
const END_PREFIX: &str = "# ec-gate-end ";
...
/// Remove all `ec-gate` blocks for a given key ID.
...
/// Remove all `ec-gate` blocks whose `expires=` timestamp has passed.
fn reap_expired_blocks(contents: &str, now: u64) -> (String, usize) {
    strip_blocks_counting(
        contents,
        |begin_line| {
            // begin_line: "# ec-gate-begin <key_id> expires=<ts>"
