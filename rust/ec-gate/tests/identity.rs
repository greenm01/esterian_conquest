...
fn save_and_load_identity_via_file() {
    let dir = std::env::temp_dir().join("ec-gate-identity-test");
...
fn save_identity_creates_parent_directory() {
    let dir = std::env::temp_dir().join("ec-gate-identity-mkdir-test");
...
fn save_identity_is_atomic_no_tmp_leftover() {
    let dir = std::env::temp_dir().join("ec-gate-identity-atomic-test");
    fs::create_dir_all(&dir).unwrap();
    let path = dir.join("identity.kdl");
