...
fn rasterize_glyph(
    ...
    if let Some(image) = rasterize_terminal_primitive(*cached, ch) {
        return (*cached, image);
    }
...
fn rasterize_terminal_primitive(
    cached: CacheRect,
    ch: char,
) -> Option<Vec<u32>> {
    let mut image = vec![0u32; cached.width as usize * cached.height as usize];

    if let Some((left, right, up, down, double)) = box_connections(ch) {
        draw_box_glyph(
            &mut image,
...
fn box_connections(ch: char) -> Option<(bool, bool, bool, bool, bool)> {
    match ch {
        ...
        '┬' => Some((true, true, false, true, false)),
        ...
        '╠' => Some((false, true, true, true, true)),
        '╣' => Some((true, false, true, true, true)),
        '╦' => Some((true, true, false, true, true)),
        '╩' => Some((true, true, true, false, true)),
...
fn draw_box_glyph(
    ...
    let horizontal_thickness = if double { 2.min(height.max(1)) } else { 1 };
    let vertical_thickness = if double { 2.min(width.max(1)) } else { 1 };
    let mid_x = width / 2;
    let mid_y = height / 2;

    if double {
        let left_col = mid_x.saturating_sub(1);
        let right_col = left_col.saturating_add(1).min(width.saturating_sub(1));
        let top_row = mid_y.saturating_sub(1);
        let bottom_row = top_row.saturating_add(1).min(height.saturating_sub(1));
    ...
    if left {
        fill_rect(image, stride, 0, mid_y, mid_x + 1, 1, color);
    }
    if right {
        fill_rect(
            image,
    ...
    if up {
        fill_rect(image, stride, mid_x, 0, 1, mid_y + 1, color);
    }
    if down {
        fill_rect(
            image,
    ...
    fill_rect(image, stride, mid_x, mid_y, 1, 1, color);
}
...
fn fill_rect(
    ...
    for row in 0..height {
        ...
        image[start..end].fill(color);
    }
}

fn rgba(alpha: u8) -> u32 {
