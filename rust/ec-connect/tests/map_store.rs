...
use ec_connect::connect::map_fetch::{MapBundlePayload, MapFilePayload};
use ec_connect::map_store::{
...
fn tmp_maps_root(name: &str) -> PathBuf {
    let path = std::env::temp_dir().join(format!("ec_connect_maps_{name}_{}", std::process::id()));
