...
use ec_data::{CampaignStore, HostedSeat, HostedSeatStatus};
use ec_gate::publish::{PublishedGameDefinitionReceipt, publish_game_definition_for_dir};
use ec_nostr::hash::sha256_hex;
use ec_nostr::hosted::{PublishedGameDefinition, parse_game_definition};
...
pub fn reissue_hosted_seat_with_publish(
    ...
    format_mutation_with_republish(
        ...
        identity_path,
        Some(format!(
            "Local seat change succeeded; if the relay publish is still stale, run `ec-sysop nostr publish --dir {}` after fixing the relay/config.",
            dir.display()
        )),
    )
}
...
fn format_mutation_with_republish(
    ...
    match maybe_republish_hosted_game(dir, config_path, identity_path) {
        ...
        Ok(RepublishOutcome::Skipped(reason)) => Ok(format!(
            "{base}\nWarning: {reason}. Run `ec-sysop nostr publish --dir {}` to refresh relay metadata.",
            dir.display()
        )),
        Err(err) => {
...
fn optional_publish_paths(
    ...
    let config_path = resolve_existing_path(config_path, ec_gate::config::config_path)?;
    let identity_path = resolve_existing_path(identity_path, ec_gate::identity::identity_path)?;
...
fn fetch_published_game_definition(
    ...
    let config = ec_gate::config::load_config(config_path)?;
    let identity = ec_gate::identity::load_identity(identity_path)?;
