...
fn apply_stardock_specs(game_data: &mut CoreGameData, specs: &[(usize, usize, u8, u16)]) {
    ...
    for (record_index_1_based, slot, kind_raw, count) in specs {
        if *record_index_1_based > total_planets || *slot >= nc_data::STARDOCK_SLOT_COUNT {
            continue;
        }
...
fn replace_player_one_reports(state: &mut nc_data::CampaignRuntimeState, year: u16) {
...
fn replace_player_one_mail(state: &mut nc_data::CampaignRuntimeState, year: u16) {
    state
        .queued_mail
...
fn full_snapshot(
    ...
    PlanetIntelSnapshot {
        ...
        known_starbase_count: Some(nc_data::active_starbase_count_at(
            game_data,
            planet.coords_raw(),
        )),
