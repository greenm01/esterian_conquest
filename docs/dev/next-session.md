...
- [x] Code-to-fleet-composition mapping recovered from kind-1 producer
      `dddb`: starbase→code 3, battleship→4, cruiser/transport/army→5,
      scouts/destroyers→6. Codes 1,2 generated by `02c0` decoder (multi-
      report from single entry). Codes 7,8 from separate producers
...
- [ ] Codes 7 and 8 producer assignment (codes 3-6 are mapped; 1,2 come
      from the 02c0 decoder; 7,8 source still unknown — likely kind-2
      producer or late-resolution path)
