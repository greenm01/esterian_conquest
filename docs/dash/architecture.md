...
For the four stacked left-column dashboard widgets (`ECONOMY`, `MY PLANETS`,
`MY FLEETS`, and `WAR RECORD`), treat the whole column as one shared
label/value grid: their `:` columns should align vertically across the overall
dashboard, not independently per widget.
...
Four stacked sections, all visible simultaneously:

- **Economy:** Treasury (total stored production across all planets),
  Avail Pts (total available production points), Prod (current production),
  Pot Prod (potential production), Revenue (this turn's tax income), Tax Rate,
  PP Gen (raw yearly production points generated by growth), % Growth (that
  same generated growth expressed against current production), plus efficiency
  and compact rank/capacity rows. The player sees spending power, capacity
  gap, and how quickly the empire is still developing.
- **My Planets:** Summary of planetary assets:
  `Tot Worlds`, `Stardocks`, `Starbases`, `Tot Armies`, `GBs`, and
  `Building` (total queued unit quantity across all owned worlds). A
  `Vulnerable` count appears in red if any owned worlds have 0 armies and 0
  batteries. Per-type `Bld ...` rows can appear below the totals when there is
  vertical room.
- **My Fleets:** Summary of space assets and their current postures:
  `Tot Fleets`, `Tot Ships`, `Docked`, `Moving`, `Combat`, `Guarding`,
  `Holding`, plus active-duty `BBs`, `CAs`, `DDs`, `SCs`, `TTs`, `ETs`, and
  `SBs` rows when there is room.
- **War Record:** Campaign-lifetime expansion and combat totals:
  `Colonies`, `Taken`, `Lost`, `Bombards`, `Invade S/F`, `Blitz S/F`,
  `Repelled`, `Ships Dest`, `Ships Lost`.

Both lists use a matching `key : value` format, with the left column sharing
one common `:` column across all four stacked widgets. Labels may be
...
The right column now has a slightly looser width budget than the left, with a
hard cap of 25 columns so passive intel widgets can breathe without widening
the whole dashboard.

- **Inbox:** Unread mailbox/report summary.
...
- **Diplomacy:** Two-line mini-table blocks sorted by current production.
  Top line shows `#<empire> | <ellipsized name>`. Bottom line shows
  `#<empire> | CP <value> | <relation>`, with the first `|` aligned between
  the two lines of each block. The current player is omitted from this passive
  list. `Relation` is relative to the current player: `Enemy` or `Neutral`.
- **Sector Detail:** Passive detail for the sector under the map crosshair.
  Owned worlds show `Planet`, `Owner`, `State`, `Curr Prod`, `Pot Prod`,
  `Stored Pts`, `ARs`, `Grnd Batt`, `SBs`, `Orbit`, `Building`, and `Docked`.
  Foreign/known worlds mirror that structure as intel allows, keeping `Intel`
  near the top and using `?` for unknown values. `Building` and `Docked` use
  the same compact unit shorthand as the rest of the TUI, for example
  `5BB 10CA 4DD`, and wrap on unit-token boundaries into extra sector-detail
  lines instead of widening the right column past its cap. Unlike the left
  summary column, these right-side rows do not pad labels to align `:` columns;
  the widget stays compact and cramped on purpose. Empty sectors show
...
- **Tab / Shift+Tab:** Cycle focus: Map → Economy → Planets → Fleets →
  War Record → Galaxy → Diplomacy → Map.
...
- Left: Economy (economic totals/ranks), My Planets (summary stats), My Fleets
  (summary stats), War Record (lifetime combat/expansion totals).
