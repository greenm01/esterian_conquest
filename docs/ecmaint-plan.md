...
The next combat-oriented maintenance scenario should not reuse the original
seed-shell bombardment target.

Current repo evidence shows that the first synthetic bombardment fixture cloned
an initialized colony seed, which was enough to trigger attacker losses but not
planet damage or generated reports. The next iteration should instead clone a
mature enemy-colony-style target and explicitly compare:

- the target `PLANETS.DAT` record
- the corresponding `DATABASE.DAT` state
- any linked empire/ownership state needed to make the world a fully valid
  defended enemy colony

This is stricter than merely cloning any developed world. A throwaway test that
reused the shipped mature colony `Dust Bowl` caused the attacker to convert
from `bombard` to `guard/blockade` on arrival, which suggests the target was
treated as friendly rather than hostile.

That mature-enemy-target correction should be applied before attempting to
generalize bombardment or invasion formulas from the synthetic combat fixtures.
