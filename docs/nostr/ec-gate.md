...
Each game directory contains a roster file that maps player seats to
Nostr public keys:

```
/srv/ec/friday-night/roster.kdl
```

Format:

```kdl
game id="friday-night" name="Friday Night EC" {
...
| `name` | Human-readable game name, shown to players in `ec-connect`. |
...
When a player redeems an invite code, `ec-gate` updates the roster:
sets the status to `claimed` and records the player's npub.

## Invite Code Management

### Generation

Invite codes are generated by `ec-sysop` when creating a game with the
`--nostr` flag, or by `ec-gate` when an admin requests new codes. Codes
...
The `@relay` suffix is appended automatically based on the server's
configured relay URL so the player can paste the full string into
`ec-connect`.
...
2. Search all game rosters for a seat with a matching code
...
```
$ ec-sysop reissue /srv/ec/friday-night --seat 2
Seat 2 reset. New invite code: jade-river@play.example.com
```
...
`ec-gate` instance. Code generation checks existing rosters and
...
`ec-gate` looks up the player's npub in the roster for the specified
game. If found, it provisions the session. If the npub is not in that
game's roster, it returns an `unknown_player` error.

### Without Invite Code, Without game-id Tag

`ec-gate` searches all game rosters for the player's npub:
...
has its own `roster.kdl` with its own invite codes and game ID slug.

On startup, `ec-gate` loads all rosters from the configured game
directories. It builds an in-memory index of npub-to-game mappings for
fast lookup during session routing.

When invite codes are claimed or seats are reissued, the roster file is
updated atomically (write to temp file, rename) to prevent corruption.

A player's npub can appear in multiple game rosters on the same server.
