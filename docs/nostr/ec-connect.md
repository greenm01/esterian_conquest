...
`nc-connect` is the player-side client binary. It manages Nostr identity,
authenticates with game servers, and launches SSH-backed `nc-game` sessions.
During the current beta, the public GitHub player download lives on the repo's
GitHub Releases page. Public `nc-connect` player archives are available for
...
Wayland from the same package. `nc-connect-cli` remains available from Cargo
or source builds for advanced manual workflows, but it is not part of the
normal player handoff or the GUI release archive. Like `nc-game` and
`nc-sysop`, it should currently be treated as beta-quality software and
...
[nc-game launches — full TUI session]
```
...
| Type | Description |
|------|-------------|
| `local` | Auto-generated by nc-connect on first launch |
...
| `maps-dir` | Optional root directory for downloaded starmap bundles. If omitted, `nc-connect` uses the platform default local data directory. |
| `server` | Named bookmark for a game server. `host` is required; `port` defaults to 22. |
| `default` | Name of the server bookmark to use when `nc-connect-cli` is invoked in direct mode with no argument and there is only one game cached for that server. |
| `lock-timeout-minutes` | Idle timeout for the local `nc-connect` shell. Default `5`. Set `0` to disable idle locking. |

Individual server bookmarks do not carry their own relay URL. All servers
on the same `nc-connect` installation use the top-level relay. If a
...
game is missing its per-game relay, `nc-connect` can prompt once for the
correct relay and then save it back onto that cached row.

### Relay Resolution Priority

When connecting, `nc-connect` resolves the Nostr relay URL from these
sources, in order:
...
the in-game claim/save path, `nc-connect` writes a cache entry so the picker
...
After a successful hosted session ends, `nc-connect` performs one lightweight
...
```
cargo run -q -p nc-connect --bin nc-connect-cli -- <args...>
```
...
Supply the lower-level overrides through `nc-connect-cli` when the sysop does
...
presses Enter. `nc-connect` resolves the server, runs the Nostr
...
`nc-connect` does not refresh maps automatically on normal reconnects.
Players fetch once on first join, then only when they explicitly ask to
download again.

### Post-Session Behavior

When a game session ends (player quits `nc-game` or connection drops),
...
In CLI direct mode (`nc-connect-cli friday`), the process exits after the game
...
`nc-connect` includes the game's `id` slug in the 30501 SessionRequest
as a `game-id` tag. `nc-gate` uses this to route directly to the correct
...
1. `nc-connect` sends a 30501 SessionRequest with no `game-id` tag.
2. `nc-gate` finds multiple hosted-game matches for the npub.
3. `nc-gate` returns a 30503 SessionError with error code
   `multiple_games` and a game list in the encrypted payload.
4. `nc-connect` displays the game list as a picker:
...
5. Player selects a game. `nc-connect` retries with the `game-id` tag.
6. Both games are added to the local cache for future disambiguation.

### CLI Direct Mode Disambiguation

In CLI direct mode (`nc-connect-cli friday`), if the server has multiple games
for the player and no cached game ID, `nc-connect` falls back to a
...
8. Wait for a response from `nc-gate` (timeout: 15 seconds).
...
15. The player is now in `nc-game`.
...
The packaged desktop player keeps the SSH-backed `nc-game` session inside the
`nc-connect` window. It forwards keyboard input to the SSH channel, renders the
remote ANSI/VT terminal stream in-process, and forwards resize events as SSH
window-change requests so `nc-game` sees the correct terminal dimensions.

This is transparent to the player. The experience is still the classic
`nc-game` session, with the latency of the SSH connection.

### GUI and CLI Paths

Packaged desktop builds use the native GUI frontend for both the picker and the
live session. The Cargo/source-only `nc-connect-cli` companion keeps the older
...
When connecting, `nc-connect` needs to know which Nostr relay to use for
...
1. `--maps-dir <PATH>` on the current `nc-connect` invocation
